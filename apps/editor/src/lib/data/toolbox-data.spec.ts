...
import { getAllShapeCategories, getShapeCategoryByProfile } from './toolbox-data';
import { ProfileName } from '../types';

const allShapeCategories = getAllShapeCategories();

describe('Toolbox Data Structure', () => {
	it('should have shape categories', () => {
		expect(allShapeCategories).toBeDefined();
		expect(allShapeCategories.length).toBeGreaterThan(0);
	});

	it('should have valid structure for all categories', () => {
		allShapeCategories.forEach((category) => {
	...
	it('should have unique category IDs', () => {
		const ids = allShapeCategories.map((cat) => cat.id);
	...
	it('should have profile field when specified', () => {
		const categoriesWithProfiles = allShapeCategories.filter((cat) => cat.profiles !== undefined);
	...
	it('should have expected basic shapes category', () => {
		const basicCategory = allShapeCategories.find((cat) => cat.id === 'basic');
	...
	it('should have expected flowchart category', () => {
		const flowchartCategory = allShapeCategories.find((cat) => cat.id === 'flowchart');
