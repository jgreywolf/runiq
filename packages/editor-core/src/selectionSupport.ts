export interface WarningDetailLike {
	message: string;
}
...
export function mergeWarnings(
	warningDetails: WarningDetailLike[],
	...
	for (const warning of merged) {
		if (!warning || detailMessages.has(warning) || unique.includes(warning)) continue;
		unique.push(warning);
	}
...
export function collectSelectedIds(selection: SelectionSnapshotLike): string[] {
	return [
...
export function collectSelectedIdSet(selection: SelectionSnapshotLike): Set<string> {
	...
	for (const id of selection.selectedNodeIds) ids.add(id);
	for (const id of selection.selectedEdgeIds) ids.add(id);
	return ids;
}
