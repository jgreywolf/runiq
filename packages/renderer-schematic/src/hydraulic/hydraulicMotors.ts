...
export const motorHydraulic = createSymbol(
  ...
  (cx, cy) => {
    ...
    return `
      ...
      ${renderConnectionLine(cx, cy - radius, cx, cy - radius - 7, { strokeWidth: 3 })}
      <text x="${cx + 12}" y="${cy - radius - 5}" font-size="7" fill="currentColor">shaft</text>

      <!-- Connection lines -->
      ${renderConnectionLine(cx - radius - 7, cy, cx - radius, cy, { strokeWidth: 2.5 })}
      ${renderConnectionLine(cx + radius, cy, cx + radius + 7, cy, { strokeWidth: 2.5 })}
    `;
  }
);
