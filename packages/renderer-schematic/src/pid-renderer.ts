 ...
 */
export function renderPID(profile: PIDProfile, options: PIDRenderOptions = {}): PIDRenderResult {
	const {
		width = 1200,
		height = 800,
		gridSize = 50,
	...
	const { equipmentLayouts, instrumentLayouts } = calculateLayout(
		profile,
		width,
		height,
		spacing,
	...
	if (showGrid) {
		svgContent += renderGrid(width, height, gridSize);
	}

	// Render process lines first (behind equipment)
	svgContent += renderLines(profile.lines || [], equipmentLayouts, instrumentLayouts, warnings);
...
function calculateLayout(
	...
	spacing: number,
	warnings: string[]
): { equipmentLayouts: EquipmentLayout[]; instrumentLayouts: InstrumentLayout[] } {
	...
	const instrumentStartY = margin + Math.ceil(equipmentLayouts.length / itemsPerRow) * spacing + spacing;
	...
	return { equipmentLayouts, instrumentLayouts };
}
 ...
 * Render equipment symbols
 */
function renderEquipment(layouts: EquipmentLayout[], showTags: boolean, showProperties: boolean): string {
 ...
 */
function renderInstruments(layouts: InstrumentLayout[], showTags: boolean): string {
...
function renderLines(
	...
	for (const line of lines) {
		...
		if (!from || !to) {
			warnings.push(`Could not find equipment/instrument for line: ${line.from?.equipment} -> ${line.to?.equipment}`);
			continue;
		}
		
		// Get line class based on type
		const lineClass = line.type === 'signal' ? 'pid-line-signal' : 
		                  line.type === 'electrical' ? 'pid-line-electrical' :
		                  line.type === 'utility' ? 'pid-line-utility' : 'pid-line';
 ...
 * Render control loop annotations
 */
function renderLoops(loops: PIDLoop[], instrumentLayouts: InstrumentLayout[]): string {
	...
	for (const loop of loops) {
		const controller = loop.controller ? instrumentMap.get(loop.controller) : undefined;
		if (controller) {
