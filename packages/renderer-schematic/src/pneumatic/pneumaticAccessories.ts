...
export const airSource = createSymbol(
  ...
  (cx, cy) => {
    ...
    return `
      ...
      <!-- Output line -->
      ${renderConnectionLine(cx + radius, cy, cx + radius + 10, cy, { strokeWidth: 2 })}
    `;
  }
);
...
export const compressor = createSymbol(
  ...
  (cx, cy) => {
    ...
    return `
      ...
      <!-- Inlet arrow -->
      ${renderConnectionLine(cx - radius - 5, cy, cx - radius, cy, { strokeWidth: 2 })}
...
export const checkValvePneumatic = createSymbol(
  ...
  (cx, cy) => {
    ...
    return `
      ...
      <!-- Input/output lines -->
      ${renderConnectionLine(cx - size / 2 - 5, cy, cx - size / 2, cy, { strokeWidth: 2 })}
      ${renderConnectionLine(cx + size / 2, cy, cx + size / 2 + 5, cy, { strokeWidth: 2 })}
    `;
  }
);
...
export const quickExhaust = createSymbol(
  ...
  (cx, cy) => {
    ...
    return `
      ...
      ${renderConnectionLine(cx - 10, cy, cx - radius, cy, { strokeWidth: 2 })}
      ${renderConnectionLine(cx + 5, cy - 8, cx + 12, cy - 12, { strokeWidth: 2 })}
      ${renderConnectionLine(cx + 5, cy + 8, cx + 12, cy + 12, { strokeWidth: 2 })}

      <!-- Connections -->
      ${renderConnectionLine(cx, cy - radius - 5, cx, cy - radius, { strokeWidth: 2 })}
      ${renderConnectionLine(cx, cy + radius, cx, cy + radius + 5, { strokeWidth: 2 })}
    `;
  }
);
...
export const exhaustPneumatic = createSymbol(
  ...
  (cx, cy) => {
    return `
      ...
      ${renderConnectionLine(cx - 10, cy - 15, cx - 5, cy - 13, { strokeWidth: 1.5 })}
      ${renderConnectionLine(cx, cy - 17, cx, cy - 12, { strokeWidth: 1.5 })}
      ${renderConnectionLine(cx + 10, cy - 15, cx + 5, cy - 13, { strokeWidth: 1.5 })}

      <!-- Connection line -->
      ${renderConnectionLine(cx, cy, cx, cy + 5, { strokeWidth: 2 })}
    `;
  }
);
  ...
  (cx, cy) => {
    ...
    return `
      ...
      ${[-8, -4, 0, 4, 8].map(offset =>
        renderConnectionLine(cx - width / 2, cy + offset, cx + width / 2, cy + offset, { strokeWidth: 1 })
      ).join('\n      ')}

      <!-- Connection line -->
      ${renderConnectionLine(cx, cy + height / 2, cx, cy + height / 2 + 10, { strokeWidth: 2 })}
...
export const pressureGaugePneumatic = createSymbol(
  ...
  (cx, cy) => {
    ...
    return `
      ...
      <!-- Connection line -->
      ${renderConnectionLine(cx, cy + radius, cx, cy + radius + 3, { strokeWidth: 2 })}
    `;
  }
);
...
export const proximitySensor = createSymbol(
  ...
  (cx, cy) => {
    ...
    return `
      ...
      ${renderConnectionLine(cx - 3, cy, cx + 3, cy, { strokeWidth: 1.5 })}
      ${renderConnectionLine(cx, cy - 3, cx, cy + 3, { strokeWidth: 1.5 })}
