...
describe('Demo: New Features (Rotation + Orthogonal Routing)', () => {
  it('should generate H-Bridge motor driver with rotation and orthogonal routing', () => {
    ...
    const outputPath = join(process.cwd(), 'src', '__tests__', 'output-h-bridge-demo.svg');
    try {
      ...
      console.log(`   - Junction dots: ${(result.svg.match(/schematic-junction/g) || []).length}`);
  ...
  it('should generate voltage regulator with mixed rotations', () => {
    const profile: ElectricalProfile = {
      ...
      parts: [
        ...
        { ref: 'C1', type: 'C', params: { value: '100u', rotation: 0 }, pins: ['VIN', 'GND'] },
        { ref: 'R1', type: 'R', params: { value: '100', rotation: 90 }, pins: ['VIN', 'VOUT'] },
        { ref: 'D1', type: 'D', params: { model: '1N4148', rotation: 180 }, pins: ['VOUT', 'GND'] },
        { ref: 'C2', type: 'C', params: { value: '10u', rotation: 270 }, pins: ['VOUT', 'GND'] },
      ],
