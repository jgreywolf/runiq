...
describe('P&ID warning checks', () => {
  it('warns when instrument loop does not match tag', () => {
    const text = `
      pid "Loop Mismatch" {
        instrument FT-101 type:flowTransmitter range:(0,100) unit:mA3/h loop:202
      }
    `;
  ...
  it('warns when loop controller tag does not match loop number', () => {
    const text = `
      pid "Loop Mismatch" {
        loop 101 controlled_variable:flow setpoint:10 unit:mA3/h controller:FIC-202
      }
    `;
  ...
  it('does not warn when tags and loops align', () => {
    const text = `
      pid "Loop Match" {
        instrument FT-101 type:flowTransmitter range:(0,100) unit:mA3/h loop:101
        loop 101 controlled_variable:flow setpoint:10 unit:mA3/h controller:FIC-101
      }
    `;
