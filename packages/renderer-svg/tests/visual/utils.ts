...
export function renderDiagram(dslCode: string): string {
	...
	if (!parseResult.success || !parseResult.diagram) {
		const errorMessages = parseResult.errors?.map((e: unknown) => {
 ...
 */
export function createShapeTestDiagram(shapeId: string, label: string = 'Test'): string {
	return `diagram "test" {
 ...
 */
export function createComplexDiagram(shapes: Array<{ id: string, shape: string, label: string }>, edges?: Array<{ from: string, to: string }>): string {
	const shapeDeclarations = shapes
		.map(s => `  shape ${s.id} as @${s.shape} label: "${s.label}"`)
		.join('\n');
	
	const edgeDeclarations = edges
		? '\n' + edges.map(e => `  edge ${e.from} -> ${e.to}`).join('\n')
		: '';
 ...
 */
export async function generateShapeBaselines(page: Page, shapes: string[]): Promise<void> {
 ...
 */
export async function compareScreenshots(page: Page, dslCode: string, snapshotName: string): Promise<void> {
