...
function inferTags(entry, content) {
  ...
  const keywordMap = {
    ...
    charts: ['chart', 'bar-chart', 'bar-chart', 'bar', 'pie', 'venn', 'radar', 'radarChart', 'line-chart', 'lineChart'],
    glyphset: ['glyphset', 'chevron', 'columnlist', 'chevronlist', 'chevron-list', 'chevronList', 'cluster', 'matrix', 'orgchart', 'org-chart'],
    ...
    control: ['hydraulic', 'electrical', 'pneumatic', 'control', 'transfer-function', 'integrator'],
  };
  ...
  title.split(/\W+/).slice(0,6).forEach((w) => { if (w && w.length>3) tags.add(w); });
